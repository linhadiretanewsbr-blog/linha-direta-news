...
import { Menu, Search, X, ChevronRight, Clock, User, Share2, MessageCircle, TrendingUp, Home, Lock, Plus, Save, Trash2, LogOut, RefreshCw, AlertCircle, WifiOff } from 'lucide-react';
...
const FALLBACK_NEWS = [
  {
    ...
    content: "Conteúdo de demonstração ativo. A conexão com a API falhou, provavlemente devido a restrições de CORS (Cross-Origin Resource Sharing). Para corrigir: Vá ao painel do Sanity > API > CORS Origins e adicione a URL do seu site.",
...
const fetchSanityNews = async () => {
  // Query GROQ para buscar items do tipo "news", ordenados por criação
...
// 2. Criar Notícia (Escrita)
const createSanityNews = async (article, token) => {
  const mutations = {
    mutations: [
      {
        create: {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex justify-between items-center h-20">
        ...
        <nav className="hidden md:flex space-x-6">
          {view === 'admin' ? (
             <span className="text-red-600 font-bold flex items-center"><Lock className="w-4 h-4 mr-1"/> MODO ADMINISTRADOR (SANITY CONNECTED)</span>
          ) : (
...
// --- PAINEL ADMIN (Com Integração Sanity) ---
...
const LoginScreen = ({ onLogin }) => {
  ...
  const handleAuth = (e) => {
    e.preventDefault();
    if (token.length > 20) { // Validação básica de tamanho de token
      onLogin(token);
    } else {
...
const AdminPanel = ({ news, setNews, token, onLogout, refreshNews }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await createSanityNews(formData, token);
      await refreshNews(); // Recarregar lista do servidor
      alert("Notícia enviada para o Sanity com sucesso!");
      setFormData({...formData, title: "", excerpt: "", content: ""});
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    // Evitar deletar dados de fallback
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-6 rounded shadow border-t-4 border-[#FFD700]">
          <h3 className="text-xl font-bold mb-4 flex items-center text-gray-800"><Plus className="w-5 h-5 mr-2"/> Publicar no Sanity</h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold uppercase text-gray-500 mb-1">Conteúdo</label>
            ...
            <button disabled={loading} type="submit" className={`w-full text-white font-bold py-3 rounded flex items-center justify-center ${loading ? 'bg-gray-400' : 'bg-[#1B5E20] hover:bg-green-800'}`}>
              {loading ? 'Enviando...' : <><Save className="w-5 h-5 mr-2" /> Publicar Notícia</>}
            </button>
          </form>
        </div>
